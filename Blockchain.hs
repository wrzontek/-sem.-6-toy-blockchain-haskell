...
mineBlock miner parent txs = Block {blockHdr = header {nonce = foundNonce}, blockTxs = txs}
  where
    header = BlockHeader {parent = parent, coinbase = coinbaseTx miner, txroot = rootHash, nonce = undefined} -- nonce undefined bo będzie zastąpiony przez foundNonce
    coin = coinbaseTx miner
    rootHash = treeHash $ buildTree (coin:txs)
    foundNonce = findNonce header 0
    
    findNonce :: BlockHeader -> Int -> Hash
    findNonce header n
      | validNonce (header {nonce = newNonce}) = newNonce
      | otherwise = findNonce header (n + 1)
      where newNonce = hash n
...
validChain p = isJust $ verifyChain p

verifyChain :: [Block] -> Maybe Hash
verifyChain [] = Just 0
verifyChain p@(h:t) = go (hash $ blockHdr h) p
  where
    go f [] = Just f
    go f [x] = if isJust $ verifyBlock x 0 then Just f else Nothing
    go f (x:y:xs) = if isJust $ verifyBlock x (hash $ blockHdr y) then go f xs else Nothing
...
mineTransactions miner parent txs = (block, getReceipts (hash $ blockHdr block) txs tree)
  where
    block = mineBlock miner parent txs
    tree = buildTree $ coinbase (blockHdr block) : txs

    getReceipts :: Hash -> [Transaction] -> Tree Transaction -> [TransactionReceipt]
    getReceipts _ [] _ = []
    getReceipts h (tx:txs) tree
      | isJust proof = TxReceipt {txrBlock = h, txrProof = justToValue proof} : getReceipts h txs tree
      | otherwise = undefined -- właśnie to drzewo zbudowaliśmy więc powinien być dobry proof
      where
        proof = buildProof tx tree
        justToValue :: Maybe a -> a
        justToValue (Just x) = x
        justToValue _ = undefined
